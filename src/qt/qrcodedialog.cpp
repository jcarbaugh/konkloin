...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("KON")
    updateDisplayUnit();
}
...
{
    QString ret = QString("konkloin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non KON unit input in lnReqAmount, we generate the URI with KON as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::KON, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
